...
df = pd.read_csv('counts_filtered.csv')
...
# =============================================================================
# # 2.2 Scatterplot for each year (color-coded by total number of vehicles)
# #plt.figure(figsize=(12, 10))
# for i, year in enumerate(df[year_column].unique(), 1):
#     plt.subplot(2, 3, i)  # Adjust layout for 2 rows and 3 columns
#     year_data = df[df[year_column] == year]
#     sns.scatterplot(x='longitude', y='latitude', hue='total_vehicles', data=year_data, palette='viridis', size='total_vehicles', sizes=(20, 200))
#     
#     plt.title(f'Scatterplot of Lat-Long with Total Vehicles in {year}')
#     plt.xlabel('Longitude')
#     plt.ylabel('Latitude')
#     plt.colorbar(label='Total Number of Vehicles')
#     plt.legend(title='Total Vehicles', loc='upper right')
# 
# plt.tight_layout()
# plt.show()
